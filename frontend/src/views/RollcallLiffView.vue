...
//初始化
onMounted(async () => {
  // 宣告一個旗標，用來判斷是否正在執行自動轉跳
  // 如果正在轉跳，我們就不在 finally 把 Loading 關掉，避免畫面閃爍
  ...
  try {
    ...
    // (注意：這裡不能用 initError，因為我們定義的是 message)
    showMessage("系統載入異常：" + err.message);
    
  } finally {
    // ★★★ 關鍵修正：安全地關閉 Loading ★★★
    // 只有在「沒有要進行轉跳」的情況下，才關閉 Loading
    // 這樣如果是轉跳中，使用者會看到轉圈圈直到瀏覽器切換，體驗較好
    if (!isRedirecting) {
